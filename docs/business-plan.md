...
Last updated: 2025-08-24 · Owner: Andreas · App: https://nordlead-console.vercel.app · Repo: https://github.com/AndreasHartov/nordlead-console-nextjs

Purpose: Single source of truth for the business, technical rollout, and day-to-day operations. It’s written so a new chat (or a human) can resume instantly without guessing context. If a newer decision is adopted during execution, the newer decision takes precedence.
...
- Execution Cards: each has a goal, clickable links, and a success check.  
...
- Add /docs/status.json with current phase/step (see Status Checkpoint).
...
- Success redirect on every link → /success?p=...&cs={CHECKOUT_SESSION_ID}.

2.2 Support/Branding/Receipts (live) — ✅ complete  
- Public details & support URL → https://dashboard.stripe.com/settings/public  
  • Current support email in Stripe: trade4@hartov.dk (kept until mailbox is live).  
- Branding (logo/color) → https://dashboard.stripe.com/settings/branding  
  • Wordmark “NordLead” (navy #0F172A, transparent), Icon rounded square (navy bg #0F172A, white “NL”), 256×256.  
- Email receipts → https://dashboard.stripe.com/settings/emails  
  • ON: Successful payments, Refunds; test delivered to trade4@hartov.dk.  
- Notifications → https://dashboard.stripe.com/settings/communication-preferences  
  • ON: payment succeeded, payment reviews (elevated risk), disputes, webhooks failing, payouts notifications.  
- Payout cadence verify → https://dashboard.stripe.com/settings/payouts  
  • Weekly — Monday; first-payout timing per Stripe.
...
Preflight — DNS verify & flush — ✅ done (2025-08-24)  
- WhatsMyDNS A/NS/CNAME good; local DNS flushed; Vercel Domains = Valid; browser loads apex and `www` 308→apex.
...
- One landing per trade with: validation (phone/email), geo capture, fast form, optional media, trust badges, transparent pricing & refund policy.  
...
- Branding: https://dashboard.stripe.com/settings/branding  
  • Wordmark “NordLead” + Icon rounded 256 applied.  
- Receipts: https://dashboard.stripe.com/settings/emails  
  • Successful payments = ON; Refunds = ON.  
- Notifications: https://dashboard.stripe.com/settings/communication-preferences  
...
Goal: Switch Stripe/Public details & receipts to support@nord-lead.dk after the mailbox exists. Domain purchased and pointed; mailbox is deferred to minimize spend until launch.

Status today:
- Domain: nord-lead.dk (nordlead.dk taken).  
- Nameservers: Vercel (`ns1.vercel-dns.com`, `ns2.vercel-dns.com`).  
- DNS verified (global + local). Vercel Domains show Valid; browser resolves apex and 308 from www.  
- DNSSEC: none.  
- Mailbox: deferred; Stripe support email stays trade4@hartov.dk until launch.

When to flip mailbox: at public site launch day (then update Stripe → Settings → Public details to support@nord-lead.dk).
...
Domain: nord-lead.dk delegated to Vercel; DNS verified; www → 308 → apex.  
...
What’s next: Phase 3 — Scaffold console tabs and stub pages.
...
2025-08-24: DNS for nord-lead.dk verified; Vercel Domains valid; www → 308 → apex. Workspace/mailbox deferred; Stripe support email stays trade4@hartov.dk. Proceed to Phase 3 scaffolding.  ← NEW
